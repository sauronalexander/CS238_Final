...
#test_img_all = images_all[:2,:,:,:]
#test_act_all = actions_all[:2,:]
# images_all = images_all.astype('float32') / 255
...
#print(test_act_all, model.predict(test_img_all[:2,:,:,:]))
...
while True:
    ...
    for i in range(steps):
        ...
        # cv2.imshow('image',img_reshape(ob.img))
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        print(act, get_teacher_action(ob))
        ob, reward, done, _ = env.step(act)
        if done is True:
            break
        else:
